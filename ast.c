...
int to_postfix(const REQ *in, int in_n, REQ *out, int *out_n) {
    ...
    while (top >= 0) {
        if (stack[top].type != OPERATOR) return 0;
        out[j++] = stack[top--];
    }
    *out_n = j;
    return 1;
...
int isNumber(Node *node) {
    ...
    for (int i = 0; node->value[i]; i++) {
        if (!isdigit(node->value[i])) return 0;
    }
    return 1;
}

int isVariable(Node *node) {
    if (node == NULL) return 0;
    if (node->value == NULL) return 0;
    for (int i = 0; node->value[i]; i++) {
        if (!isalpha(node->value[i])) return 0;
    }
    return 1;
}
...
Node *transform(Node *node) {
    ...
    if (node->op == '+' && isNumber(node->left) && isNumber(node->right)) {
        int result = atoi(node->left->value) + atoi(node->right->value);
        char buf[32];
        sprintf(buf, "%d", result);
        Node *newNode = createValue(buf);
        freeTree(node->left);
        freeTree(node->right);
        free(node);
        return newNode;
